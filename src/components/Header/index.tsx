...
import { ChainId, NATIVE_TOKEN } from 'constants/chains'
import { colors, HideExtraSmall } from '../../theme'
...
const HeaderFrame = styled.div<{ showBackground: boolean }>`
  ...
  background-image: ${({ theme }) => `linear-gradient(to bottom, transparent 50%, ${theme.bg1} 50% )}}`}
...
const HeaderElement = styled.div<{ flex?: string }>`
  display: flex;
  align-items: center;
  ...
  return (
    <HeaderFrame showBackground={scrollY > 45}>
      ...
      <HeaderRow>
        ...
        <HideLarge style={{ position: 'absolute', right: 20, top: 13 }}>
          {chainId && NETWORK_LABELS[chainId] && (
            <NetworkCard title={NETWORK_LABELS[chainId]}>{NETWORK_LABELS[chainId]}</NetworkCard>
          )}
        </HideLarge>
        <StyledNavLink to={'/'} style={{ fontSize: 30, alignItems: 'center' }}>
          V A P O R
        </StyledNavLink>
        {/* 😶‍🌫️ */}
      </HeaderRow>
      ...
      <HeaderControls>
        <HeaderElement flex={'1'}>
          <HideSmall>
            {chainId && NETWORK_LABELS[chainId] && (
              <NetworkCard title={NETWORK_LABELS[chainId]}>{NETWORK_LABELS[chainId]}</NetworkCard>
            )}
          </HideSmall>
          {pathname == '/bridge' ? null : (
            <AccountElement active={!!account} style={{ pointerEvents: 'auto' }}>
              {account && userEthBalance && chainId && NATIVE_TOKEN[chainId] ? (
                <HideExtraSmall>
                  <BalanceText style={{ flexShrink: 0 }} pl="0.75rem" pr="0.5rem" fontWeight={500}>
                    {userEthBalance?.toSignificant(4)}{' '}
                    <span style={{ color: colors(true).secondary1 }}>{NATIVE_TOKEN[chainId]}</span>
                  </BalanceText>
                </HideExtraSmall>
              ) : null}
              <Web3Status />
