...
import { WTLOS, VAPOR } from 'constants/tokens'
...
export function useWETHContract(withSignerIfPossible?: boolean) {
  ...
  return useContract<Weth>(chainId ? WTLOS[chainId]?.address : undefined, WETH_ABI, withSignerIfPossible)
}
...
export function useUniContract() {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId ? VAPOR[chainId]?.address : undefined, UNI_ABI, true)
}
